...
void MFT1A_Handler(void)
{
  if (MFT_StatusIT(MFT1, MFT_IT_TNA) != RESET )
  {
    ...
    /* Set reload register to generate signal with frequency 2x input signal frequency */   
    MFT1->TNCNT1 = input_period;
  }
  ...
  if (MFT_StatusIT(MFT1, MFT_IT_TNB) != RESET )
  {
    ...
    /* Save number of CPU clock period from capture register to variable */
    input_period = (MFT1->TNCRA)>>1;
  }
}
